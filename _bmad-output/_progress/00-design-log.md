...
| 2026-07-13 | Hero made personal (Rohan): "Hello — I'm" greeting eyebrow + portrait (hero.png) in museum-arch frame; desktop = text left / portrait right, mobile = portrait-first centered. Greeting is code-side microcopy (AD-1 note: same class as stat labels/room cue). Spec 1.1 layout superseded on this point | Phase 5: Evolution | Rohan + Claude |
| 2026-07-13 | Poster wall refined (Rohan: align stats, use the PNG creatively, no fig caption, Apple-grade polish): cut-out portrait rises from a warm arch backdrop (theme-adaptive color-mix gradient + wall-wash halo for dark-mode hair separation); stats = three-bay hairline placard strip the portrait stands on (equal columns, aligned numerals); short-viewport compression keeps the full proof in the first viewport | Phase 5: Evolution | Rohan + Claude |
| 2026-07-13 | Entrance reimagined as "the poster wall" (Rohan: non-template, Dribbble-grade): poster-scale name (new --text-poster token), portrait standing on the stat/cue hairline rail like an exhibit on its plinth (≥900px), "fig. 01 — the artist" caption (mobile), curatorial vertical side rails ≥1200px. Verified: desktop geometry (name clears portrait, portrait foot = rail line ±1px), mobile + dark visuals, no h-scroll, LCP payload unchanged | Phase 5: Evolution | Rohan + Claude |
